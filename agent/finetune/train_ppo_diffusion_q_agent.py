...
class TrainPPODiffusionQAgent(TrainPPOAgent):
        ...
        while self.itr < self.n_train_itr:
            ...
            if not eval_mode:
                with torch.no_grad():
                    ...
                    for obs in obs_ts:
                        # forward pass to sample actions
                        num_repeat = 10
                        obs = {'state': torch.repeat_interleave(obs['state'], num_repeat, dim=0)}
                        actions = self.model(
                            cond=obs,
                            deterministic=False,
                            return_chain=False,
                        ).trajectories[:, : self.act_steps]
                        values = self.model.critic(obs, actions).cpu().numpy().flatten()
                        values = np.mean(values.reshape(-1, num_repeat), axis=1)
                        ...
                        if t == self.n_steps - 1:
                            ...
                            obs_venv_ts = {
                                "state": torch.repeat_interleave(
                                    obs_venv_ts["state"], num_repeat, dim=0
                                )
                            }
                            actions = self.model(
                                cond=obs_venv_ts,
                            ...
                            nextvalues = (
                                self.model.critic(obs_venv_ts, actions)
                                .reshape(-1, num_repeat)
                                .cpu()
                                .numpy()
                            )
                            nextvalues = np.mean(nextvalues, axis=1)
                        else:
                ...
                actions_k = einops.rearrange(
                    torch.tensor(action_trajs, device=self.device).float(),
                    "s e t d -> (s e) t d",
                )
                returns_k = (
                ...
                for update_epoch in range(self.update_epochs):
                    ...
                    for batch in range(num_batch):
                        ...
                        actions_b = actions_k[batch_inds_b]
                        ...
                        # num_repeat = 10
                        # obs = {
                        #     'state': torch.repeat_interleave(obs_b['state'], num_repeat, dim=0)
                        # }
                        # actions = self.model(
                        #     cond=obs,
                        #     deterministic=False,
                        #     return_chain=False,
                        # ).trajectories[:, : self.act_steps]
                        newvalues_b = self.model.critic(obs_b, actions_b)
                        # newvalues = newvalues.reshape(-1, num_repeat)
                        # newvalues_b = torch.mean(newvalues, dim=1)
